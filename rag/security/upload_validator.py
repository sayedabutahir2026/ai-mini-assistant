...
from rag.settings import ALLOWED_MIME_TYPES, AUDIT_LOG_FILE, MAX_UPLOAD_SIZE_MB

logger = logging.getLogger(__name__)

# A single audit record — annotated for clarity
...
def check_and_record_upload(file_path: Path) -> FileAuditRecord:
    ...
    # ── Step 1: MIME check via magic bytes ───────────────────────────────────
    ...
    if detected is not None:
        ...
        # filetype returns None for plain text — only allow .txt or .md extensions here
